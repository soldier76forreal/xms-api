...
router.post("/newCustomer" , verify  , async (req , res , next)=>{
    ...
    for(var i = 0 ; req.body.phoneNumberInformation.length > i ; i++){
        const item = await customers.findOne({'contactInfo.phoneNumbers.number':req.body.phoneNumberInformation[i].number , 'contactInfo.phoneNumbers.countryCode':req.body.phoneNumberInformation[i].countryCode})
        if(item === null){
            error.status = false
            error.msg = ''
            phoneNumbers.push(
            ...
            error.status = true
            error.msg = 'Phone number is available in database'
        }
    }
    ...
    if( req.body.facebook !== null){
        for(var j = 0 ; req.body.facebook.length > j ; j++){
            facebooks.push(
                {
                    facebook:req.body.facebook[j].facebook,
                    logsStatus:{status:'created' , msg:'customer facebook document created!'}
                }
            )
        }
    }
    ...
    var botims = [];
    if( req.body.botim !== null){
        for(var j = 0 ; req.body.botim.length > j ; j++){
            botims.push(
                {
                    botim:req.body.botim[j].botim,
                    logsStatus:{status:'created' , msg:'customer botim document created!'}
                }
            )
        }
    }
    ...
    const newCustomer = new customers({
            ...
            personalInformation:{
                ...
                companyName:req.body.personalInformation.companyName,
                personOrCompany:req.body.personalInformation.personOrCompany,
            ...
            contactInfo:{
                ...
                facebooks:facebooks.length === 0 ? null :facebooks,
                botims:botims.length === 0 ? null :botims
...
router.post("/editCustomer"  , verify , async (req , res , next)=>{
    ...
    var facebooks = [];
    if( req.body.facebook !== null){
        for(var t = 0 ; req.body.facebook.length > t; t++){
            facebooks.push(
                {
                    facebook:req.body.facebook[t].facebook,
                    logsStatus:{status:'created' , msg:'customer facebook document created!'}
                }
            )
        }
        ...
        const editCustomer = await customers.updateOne({_id:req.body.id},{
            ...
            $set:
            {
                'contactInfo':{
                    ...
                    facebooks:lengthCheck(facebooks),
                    botims:lengthCheck(botims)
                }
            }
        })
        if(error.status === true){
...
router.post("/saveNewCommunication"  , verify , async (req , res , next)=>{
    try{

        const addNewCommunication = await customers.findOneAndUpdate({_id:req.body.docId},
            {
                $push:
                {'communication':{
                    ...
                    communicationReason:req.body.communicationReason,
                    targetRequests:req.body.targetRequest,
                    communicationStatus:req.body.communicationStatus,
                    description:req.body.communicationDescription
                }
                }
            }
            );
         
        res.status(200).send('new communication added...');
...
router.post("/frequentBtnClickForCrm"  , verify , async (req , res , next)=>{

    try{
        const updateFrequentBtnClick = await customers.updateOne({_id:req.body.customerId},{
            deleteDate:null,
            "$push": { 'frequentBtnClick' : [req.body.btn] }
        })
        res.status(200).send('btn has been added!')
    }catch(error){
        console.log(error)
        res.status(400).send('error');
    }
...
router.get("/allCustomerBasedOnUser"  , verify , async (req , res , next)=>{

    try{
        var decoded = jwt_decode(req.headers.authorization);
        const allCustomers = await customers.find({inisialInsert:decoded.id , deleteDate:null})
        res.status(200).send({customer:allCustomers})
    }catch(error){
        console.log(error)
        res.status(400).send('error');
    }
