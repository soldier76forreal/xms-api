...
//get all users
  ...
  router.get('/userProfileData' , verify , async(req , res)=>{
    var decoded = jwt_decode(req.headers.authorization);
    try{ 
        const theUser = await user.findOne({_id:decoded.id}).select('jobReport , jobReportPresets , firstName , lastName , phoneNumber  ,filterMemory , insertDate , updateDate , access');
        res.status(200).send(theUser)
      }catch(err){
          console.log(err)
      }
